...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [
            ...
            if not hitsWall:
                # copy parent visited list
                visitedCorners = visited.copy()
                dState = ((nextx, nexty, visitedCorners), action, 1)
                # if next is corner that has not been visited already, add to visited
                if (nextx, nexty) in self.corners and (nextx,
                                                       nexty) not in visited:
                    visitedCorners.append((nextx, nexty))
                successors.append(dState)
            "*** YOUR CODE HERE ***"
...
def cornersHeuristic(state, problem):
    ...
    # for each unvisited corner, find the shortest manhattan distance and go to that node
    while unvisited:
...
def foodHeuristic(state, problem):
    ...
    # for each food find the shortest distance from pacman to it
    for food in foodList:
        if (node, food) not in problem.heuristicInfo:
            # calculate and store distance
            distance = mazeDistance(node, food, problem.startingGameState)
            problem.heuristicInfo[(node, food)] = distance
        else:
            # distance already calculated
            distance = problem.heuristicInfo[(node, food)]

        # if the distance is higher, update the heuristic
        if distance > heuristic:
            heuristic = distance

    # return the farthest food's distance
    return heuristic
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        distance, food = min([(util.manhattanDistance(startPosition, food), food) \
                                for food in foodList])
